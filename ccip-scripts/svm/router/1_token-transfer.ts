...
class TokenTransferCommand extends CCIPCommand<TokenTransferOptions> {
  constructor() {
    const metadata: CommandMetadata = {
      ...
      examples: [
        ...
        "yarn svm:token-transfer --fee-token link",
        "",
        "# Transfer custom token with custom amount",
        "yarn svm:token-transfer --token-mint 4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU --token-amount 50000000",
        "",
        "# Transfer to custom receiver address",
        "yarn svm:token-transfer --receiver-address 0x1234567890123456789012345678901234567890 --token-amount 25000000",
      ],
      notes: [
  ...
  protected defineArguments(): ArgumentDefinition[] {
    return [
      ...
      },
      {
        ...
        type: "string",
        description:
          "Token mint address to transfer (default: uses BnM token from config)",
  ...
  private createMessageConfig(config: any) {
    ...
    const destinationChain =
      ...
      TOKEN_TRANSFER_CONFIG.defaultDestinationChain;
    ...
    const evmReceiverAddress =
      ...
      TOKEN_TRANSFER_CONFIG.defaultEvmReceiverAddress;
    const tokenMint = this.options.tokenMint
      ? new PublicKey(this.options.tokenMint)
      : new PublicKey(config.bnmTokenMint);
    const tokenAmount =
      this.options.tokenAmount || TOKEN_TRANSFER_CONFIG.defaultTokenAmount;
    const gasLimit =
      this.options.gasLimit ?? TOKEN_TRANSFER_CONFIG.defaultGasLimit;
    const allowOutOfOrder =
      this.options.allowOutOfOrder ??
      TOKEN_TRANSFER_CONFIG.defaultAllowOutOfOrder;
  ...
  protected async execute(): Promise<void> {
    ...
    // Resolve fee token
    const feeTokenInput =
      this.options.feeToken || TOKEN_TRANSFER_CONFIG.defaultFeeToken;
    ...
    try {
      ...
      this.logger.info(
        `✅ Sent ${messageConfig.tokenAmount} tokens to ${messageConfig.evmReceiverAddress}`
      );
