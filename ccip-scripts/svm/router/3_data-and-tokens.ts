...
class DataAndTokensCommand extends CCIPCommand<DataAndTokensOptions> {
  ...
  protected defineArguments(): ArgumentDefinition[] {
    return [
      ...
      },
      {
        ...
        type: "string",
        description:
          "Token to use for fees (native, wrapped-native, link, or address)",
        example: "native",
      },
      {
        name: "gas-limit",
  ...
  private createMessageConfig(config: any) {
    ...
    const destinationChain =
      ...
      DATA_AND_TOKENS_CONFIG.defaultDestinationChain;
    ...
    const evmReceiverAddress =
      ...
      DATA_AND_TOKENS_CONFIG.defaultEvmReceiverAddress;
    const tokenMint = this.options.tokenMint
      ? new PublicKey(this.options.tokenMint)
      : new PublicKey(config.bnmTokenMint);
    const tokenAmount =
      this.options.tokenAmount || DATA_AND_TOKENS_CONFIG.defaultTokenAmount;
    const message =
      this.options.message || DATA_AND_TOKENS_CONFIG.defaultMessage;
    const numberValue =
      this.options.numberValue ?? DATA_AND_TOKENS_CONFIG.defaultNumber;
    const gasLimit =
      this.options.gasLimit ?? DATA_AND_TOKENS_CONFIG.defaultGasLimit;
    const allowOutOfOrder =
      this.options.allowOutOfOrder ??
      DATA_AND_TOKENS_CONFIG.defaultAllowOutOfOrder;
   ...
   */
  private displayTransferParameters(
    messageConfig: any,
    feeToken: PublicKey
  ): void {
    ...
    this.logger.info(
      `Message Data Length: ${messageConfig.messageData.length} bytes`
    );
  ...
  protected async execute(): Promise<void> {
    ...
    // Resolve fee token
    const feeTokenInput =
      this.options.feeToken || DATA_AND_TOKENS_CONFIG.defaultFeeToken;
    ...
    try {
      ...
      this.logger.debug(
        `Added compute budget instruction with limit: ${DATA_AND_TOKENS_CONFIG.computeUnits} units`
      );

      // Create send options exactly like the executor does
      const sendOptions: CCIPSendOptions | undefined = this.options
        .skipPreflight
        ? { skipPreflight: true }
        : undefined;
      ...
      const explorerCluster =
        config.id === "solana-mainnet" ? "" : "?cluster=devnet";
      this.logger.info(
        `Solana Transaction: https://explorer.solana.com/tx/${result.txSignature}${explorerCluster}`
      );
      if (result.messageId) {
        this.logger.info(
      ...
      this.logger.info(
        `✅ Sent ${messageConfig.tokenAmount} tokens with message: "${messageConfig.message}"`
      );
      ...
      this.logger.error(
        `❌ Failed to send data and token transfer: ${
