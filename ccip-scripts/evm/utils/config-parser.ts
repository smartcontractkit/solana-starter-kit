...
const loadEnvFile = (): EnvLoadResult => {
  ...
  console.warn('No valid .env file found in any expected location. Paths checked:');
  potentialPaths.forEach(p => console.log(`- ${p} (${fs.existsSync(p) ? 'exists' : 'not found'})`));
...
if (envResult.path) {
  console.log(`Successfully loaded environment variables from ${envResult.path}`);
} else {
  ...
  if (tokenAmountsIndex >= 0 && args.length > tokenAmountsIndex + 1) {
    try {
      ...
      const tokenAmounts = tokenAmountsStr.split(',').map(pair => {
        ...
        if (!token || !amount) {
          throw new Error(`Invalid token-amount pair format: ${pair}. Expected format: token:amount`);
        }
        return { token, amount };
      });
      ...
      console.warn(`Warning: Failed to parse --token-amounts. Format should be token1:amount1,token2:amount2,...`);
...
export function printUsage(scriptName: string): void {
  ...
  console.log("\nOptions:");
  console.log("  --fee-token <token>             Token to use for CCIP fees (native, link, wrapped)");
  console.log("  --token-amounts <list>          List of token:amount pairs (token1:amount1,token2:amount2,...)");
  console.log("  --token <address>               Token address to transfer (deprecated, use --token-amounts)");
  console.log("  --amount <amount>               Amount to transfer (deprecated, use --token-amounts)");
  console.log("  --receiver <address>            Receiver address for CCIP message");
  console.log("  --token-receiver <address>      Solana wallet to receive tokens");
  console.log("  --data <message>                Custom message data to include");
  console.log("  --compute-units <units>         Compute units for Solana execution");
  console.log("  --accounts <list>               Comma-separated list of additional accounts");
  console.log("  --account-is-writable-bitmap <value>  Bitmap for writable accounts");
  console.log("  --log-level <level>             Log level (TRACE, DEBUG, INFO, WARN, ERROR)");
  console.log("  --help, -h                      Show this help information");
  console.log("\nEnvironment variables:");
  console.log("  EVM_PRIVATE_KEY                 Private key for signing transactions");
  console.log("  EVM_RPC_URL                     (Optional) Custom RPC URL for the Ethereum Sepolia network");
} 